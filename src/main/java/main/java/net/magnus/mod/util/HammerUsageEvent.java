...
import main.java.net.magnus.mod.item.custom.EnderiteHammerItem;
...
public class HammerUsageEvent implements PlayerBlockBreakEvents.Before{
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos,
        ...
        if(mainHandItem.getItem() instanceof EnderiteHammerItem hammer && player instanceof ServerPlayerEntity serverPlayer) {
            if(HARVESTED_BLOCKS.contains(pos)) {
                return true;
            }

            for(BlockPos position : EnderiteHammerItem.getBlocksToBeDestroyed(1, pos, serverPlayer)) {
